	...
	YAHOO.extend(SOP.DynamicDropdown, Alfresco.component.Base,
	{
		...
		loadValues : function() {
            ...
            if (this.options.dependsOn.length > 0)
            {
                ...
                {
                	dependencyQuery+="&"+SOP.Dependency.Mappings[dependencyId]+"="+this.options.dependsOnValues[dependencyId];
                }
            }
            
            Alfresco.util.Ajax.request(
            {
               url: Alfresco.constants.PROXY_URI + 'com/sop/picklist/picklist?includeBlankItem=true&name='+this.options.picklistName+'&itemId='+this.options.itemId+dependencyQuery,
               method: "GET",
